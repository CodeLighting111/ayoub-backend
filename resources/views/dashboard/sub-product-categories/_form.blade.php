@php
    $cancelUrl = route('admin.sub-product-categories.index');
@endphp
    ...
    <form action="{{ $action }}" class="space-y-6" method="POST">
        ...
        <div class="space-y-2">
            ...
            <select
                ...
                required
            >
                <option disabled @selected(! old('main_product_category_id', $category->main_product_category_id)) value="">اختر الفئة الرئيسية</option>
                @foreach ($mainCategories as $mainCategory)
                    <option @selected((int) old('main_product_category_id', $category->main_product_category_id) === $mainCategory->id) value="{{ $mainCategory->id }}">
                        {{ $mainCategory->title }}
                    </option>
                @endforeach
            </select>
            @error('main_product_category_id')
                <p class="text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>

        <div class="space-y-2">
            <label class="block text-sm font-semibold text-on-surface" for="title">عنوان الفئة الفرعية <span class="text-error">*</span></label>
            <input
                @class([
